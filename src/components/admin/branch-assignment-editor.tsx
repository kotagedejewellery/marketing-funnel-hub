          ...
          >
            <form action={action} noValidate className="space-y-5">
              ...
              {assignments.map((row) => (
                ...
                >
                  <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-3 px-5 py-3 font-medium hover:bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 [&::-webkit-details-marker]:hidden">
                    <span className="min-w-0 break-words">{row.name}</span>
                    <span className="ml-auto shrink-0 rounded-full bg-secondary px-3 py-1 text-xs font-medium text-muted-foreground">
                      {!row.isBranchActive
                        ? "Cabang nonaktif"
                        : row.isActive
                          ? "Penugasan aktif"
                  ...
                  <fieldset className="border-t border-border px-5 py-5">
                    <legend className="sr-only">Pengaturan {row.name}</legend>

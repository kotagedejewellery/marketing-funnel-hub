...
related_targets:
  ["src/app/admin/(cms)/layout.tsx", "src/components/admin/navigation.tsx"]
---

# Admin CMS shell and dashboard

Mode: Operate. Marketing scans live content state, reaches frequent editing tasks, and reviews recent changes. Keep the approved destinations, role filtering, data, and sign-out behavior. Product types are managed contextually from a branch Link Bio editor rather than through a separate primary destination. Desktop-first, responsive on smaller screens.

## Direction contract

THESIS: The CMS feels like a clear working studio, not a conventional full-height sidebar beside uniform metric cards. Its bento layout distinguishes editing tasks, operational status, and truthful internal-event analytics without fabricating business data.
...
STORY: An admin sees the current site identity, active branch and WhatsApp-button counts, recent changes, and direct routes to editing. Analytics opens as a focused operational read of internal PageView, ViewContent, Contact, and LinkClick data, with a time/branch filter, comparison, funnel, daily trend, rankings, safe technical/geo aggregates, provider status, CSV export, and an inspectable event-detail view.
